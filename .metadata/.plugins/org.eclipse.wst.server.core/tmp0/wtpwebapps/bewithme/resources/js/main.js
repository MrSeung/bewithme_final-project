...
async function bookmark(id){
	try {
    const resp =  await fetch("/bookmark/getmainbookmark/"+id);
    const result = await resp.json();
    return  result;
...
//bookmark
function bookmarkPrint(){
    const bookmark_container = document.getElementById('like-container');

    bookmark(id).then(result =>{
        console.log("bookmark > Controller(DB) -> JS : " + (( result.bookmarkList.length > 0 )? "OK":"FAIL"));
        console.log("course > Controller(DB) -> JS : " + (( result.courseList.length > 0 )? "OK":"FAIL"));
    
        if(result.courseList.length > 0 && result.bookmarkList.length > 0){
            ...
            for(let cvo of result.courseList){
                ...
                for(let i of result.bookmarkList){
                	
                    if(cvo.cou_num == i){

                        let div = `<a href="/co/link?cou_num=${cvo.cou_num}"><div class="like-content">`;
                        div += `<h4> ${cnt}.  ${cvo.cou_title}</h4>`;
                        switch (cvo.sub_num) {
                            case 1:
                                div += `<p>HTML/CSS</p>`;
                                break;
                        ...
                        div += `</div></a>`;
                        bookmark_container.innerHTML += div;
                        
                        
	                    cnt++;
	                    if(cnt > 4) break loop; //출력할 갯수 (1부터시작)
                    }
                }
            }
    
        }else{
            let div = `<div class="like-content">
                            <h4>북마크한 강의가 없습니다</h4>
                        </div>`
            bookmark_container.innerHTML = div;
            console.log("북마크한 강의가 없습니다.");
        }
...
async function community(){
	try {
    const resp =  await fetch("/community/getmaincommunity/");
    const result = await resp.json();
    return  result;
...
//qna
function communityPrint(){
    const qna = document.getElementById('qna');

    community().then(result => {
        console.log("qna > Controller(DB) -> JS : " + (( result )? "OK":"FAIL"));
        qna.innerHTML = '';
        let div = `<div class="qna-header">
                        <h3>Community</h3>
                        <button type="button" onclick="location.href='/main/community'">더보기 +</button>
                    </div>`;
        div += `<a href="/community/detail?com_num=${result.com_num}"><div class="qna-content">
                    <h4>${result.com_category}</h4>
                    <p>${result.com_title}</p>
                </div></a>`
        qna.innerHTML = div;    
